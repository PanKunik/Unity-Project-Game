...
public class PlayerStats : CharacterStats
{
    ...
    {
        CreateParticle();
        InitCBT("LVL UP!", LVLUPPrefab);

        armor.SetValue(armor.GetValue() + 1);
        defaultArmor++;

        minDamage.SetValue(minDamage.GetValue() + 1);
        maxDamage.SetValue(maxDamage.GetValue() + ((int)(level * 0.35f)));
    ...
    public void SetBootProperties(int armor, float movementSpeed)
    {
        ...
        speedAnim = movementSpeed / 8;

            // playerMov.SetMovementSpeed(5);
            // speedAnim = 0;
        
        anim.SetFloat("WalkingSpeed", anim.GetFloat("WalkingSpeed") + speedAnim);
    ...
    {
        GameObject temp;
        temp = Instantiate(LvlUpParticle) as GameObject;
        temp.transform.SetParent(gameObject.transform);
        temp.transform.localPosition = LvlUpParticle.transform.localPosition;
        temp.transform.localScale = LvlUpParticle.transform.localScale;
        temp.transform.localRotation = LvlUpParticle.transform.localRotation;
        Destroy(temp, 3f);
    }
}
